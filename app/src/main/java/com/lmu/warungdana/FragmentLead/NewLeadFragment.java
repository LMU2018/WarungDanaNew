 ...
 */
public class NewLeadFragment extends Fragment implements Comparator<ListLead> {
...
*/
    ...
    private void firstLoad() {
        ...
        mApiService.listLeadPagg(idUser, 1,limit,0).enqueue(new Callback<RespListLead>() {
            @Override
            public void onResponse(Call<RespListLead> call, Response<RespListLead> response) {
                if (response.isSuccessful()) {
                    ...
                    if (response.body().getData() != null) {
                        ...
                        Collections.sort(listLeads,Collections.reverseOrder(NewLeadFragment.this));
                        listLeadAdapter.notifyDataSetChanged();
                        if (listLeads.size() >= 1) {
    ...
    private void loadMore() {
        itShouldLoadMore = false;
        mApiService.listLeadPagg(idUser, 1, limit, offset).enqueue(new Callback<RespListLead>() {
            @Override
            public void onResponse(Call<RespListLead> call, Response<RespListLead> response) {
                if (response.isSuccessful()) {
                    itShouldLoadMore = true;
